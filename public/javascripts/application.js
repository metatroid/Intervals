function showMessage(message, size){
	
	$(".notification").html("<div class='notify'><div class='msg' style='font-size: "+size+"px'><span class='icon'>W</span>"+message+"</div></div>")
...
function moveToAction(action, project, callback){
	...
	$.post( '/actions', {
		...
		callbackFunction: callback
			...
			if(action != "invoice" && action != "invoice2"){
...
//donation message
if($.cookie('panhandle') != "off"){
  $("#donate").show();
}
if($.cookie('panhandle') == "off"){
  $("#reconsider").show();
}
$(".hide_digital_panhandling").click(function(e){
	$.cookie('panhandle', "off", { expires: 60, path: '/' });
	$("#reconsider").show();
	$("#donate").hide();
	return false;
});
$(".reconsider_contribution").click(function(e){
	$.cookie('panhandle', "on", { expires: 60, path: '/' });
	$("#reconsider").hide();
	$("#donate").show();
	//showMessage("Thank you for your support", 10);
	return false;
});

	//add htttp
	
	$("#project_dev_url, #project_pro_url").focus(function(){
	...
	$('.select ul li a:not(".mainSettings")').live("click", function(event){
		...
		if(!timing){
			...
			showMessage("Please stop all running timers before changing project view.");
		}
	});
	...
	$(".mainSettings").live("click", function(event){
		...
		if(!timing){
			...
			showMessage("Please stop all running timers before changing main settings.");
		}
	});
	//end select
	...
	$(".projectInvoice").live("click", function(event){
		event.preventDefault();
		event.stopPropagation();
		project = $(this).attr("data-project_id");
		if($(".projectWrapper-"+project).find(".pBudget").length == 0){
			moveToAction("invoice", project);
		} else {
			if($(".linkWrapper").length > 0){
				...
				$(linkContainer).html("<div class='linksArrow2'></div><div class='linkContainer'><ul><li><a class='hInvoice' href='#' target='_blank' title='Generate invoice from hourly rate'>Hourly Invoice</a></li><li><a class='fInvoice' href='#' target='_blank' title='Generate invoice from flat rate'>Flat Invoice</a></li></ul></div>");
				$(this).closest(".clientControls").append(linkContainer);
				$(linkContainer).fadeIn(400);
				$("a.hInvoice").click(function(e){
					e.preventDefault();
					moveToAction("invoice", project);
				});
				$("a.fInvoice").click(function(e){
					e.preventDefault();
					moveToAction("invoice2", project);
				});
